...
function DeliveryBanner({ orders, onStatusChange, onDelete, cronStatus, onRefresh, refreshing }: {
  ...
  const deliveries = useMemo(() => {
    // Commandes avec date : filtrées et triées normalement
    ...
    // Commandes sans lien de suivi, pas encore livrées/collected
    const withoutTracking = orders
      .filter(o =>
        !o.tracking_link &&
        o.delivery_status !== 'collected' &&
        o.delivery_status !== 'delivered'
      )
      .map(o => ({ order: o, date: null as any, diffDays: 999, noTracking: true }));

    return [...withDate, ...withoutTracking];
  }, [orders]);
