        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <h1 className="text-4xl font-extrabold text-gray-900 dark:text-gray-100 tracking-tight">Mes commandes</h1>
            </div>
            <button
              onClick={() => { setEditingOrder(null); setShowForm(true); }}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition-colors shadow-sm dark:shadow-none">
          ...
          <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-gray-850 rounded-xl shadow-sm dark:shadow-none text-xs">
              ...
              <button onClick={triggerRefresh} disabled={refreshing}
                className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-950 text-blue-600 hover:bg-blue-100 transition-colors disabled:opacity-50 font-medium">
                ...
                {selectedOrder && (() => {
                  ...
                  return (
                    <div key={selectedOrder.id} className="flex-1 detail-anim space-y-3 min-w-0">
                      ...
                      <div className="bg-white dark:bg-gray-850 rounded-2xl shadow-sm dark:shadow-none overflow-hidden">
                        <div className="px-5 py-3 border-b border-gray-50 dark:border-gray-700 flex items-center justify-between">
                          <p className="text-xs font-semibold text-gray-400 dark:text-gray-300 uppercase tracking-wide">NOTE</p>
                          <button onClick={() => setEditingNote(editingNote?.id === selectedOrder.id ? null : { id: selectedOrder.id, value: selectedOrder.notes || '' })}
                          ...
                          {editingNote?.id === selectedOrder.id ? (
                            <div className="space-y-2">
                              <textarea autoFocus rows={3}
                                ...
                                className="w-full text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none"
                              />
                              <div className="flex gap-2">
                                <button onClick={() => handleSaveNote(selectedOrder.id, editingNote.value)} disabled={savingNote}
                                  ...
                                  {savingNote ? <Loader2 size={11} className="animate-spin" /> : <Check size={11} />} Enregistrer
                                </button>
                                <button onClick={() => setEditingNote(null)} className="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 text-xs hover:bg-gray-50 dark:hover:bg-gray-700">Annuler</button>
                              </div>
                            </div>
                          ) : selectedOrder.notes ? (
                            <p className="text-sm text-gray-600 dark:text-gray-300 whitespace-pre-wrap">{selectedOrder.notes}</p>
                          ) : (
                            <p className="text-xs text-gray-300 dark:text-gray-600 italic">Aucune note</p>
                          )}
                        </div>
                      </div>
                      ...
                      {selectedOrder.tracking_checkpoints && selectedOrder.tracking_checkpoints.length > 0 && (
                        <div className="bg-white dark:bg-gray-850 rounded-2xl shadow-sm dark:shadow-none overflow-hidden">
                          <div className="px-5 py-3 border-b border-gray-50 dark:border-gray-700">
                            <p className="text-xs font-semibold text-gray-400 dark:text-gray-300 uppercase tracking-wide">HISTORIQUE DE SUIVI</p>
                          </div>
                          <div className="px-5 py-4">
                            <div className="relative">
                              <div className="absolute left-[7px] top-2 bottom-2 w-px bg-gray-100 dark:bg-gray-700" />
                              <div className="space-y-4">
                                {[...selectedOrder.tracking_checkpoints].map((cp, i) => {
                                  const isFirst = i === 0;
                                  const dotColor = isFirst ? '#3b82f6' : '#d1d5db';
                                  return (
                                    <div key={i} className="flex gap-3 relative">
                                      <div className="flex-shrink-0 w-3.5 h-3.5 rounded-full border-2 mt-0.5 z-10"
                                        style={{ borderColor: dotColor, background: isFirst ? dotColor : 'transparent' }} />
                                      <div className="flex-1 min-w-0 pb-1">
                                        <p className={`text-xs font-medium leading-snug ${isFirst ? 'text-gray-900 dark:text-gray-100' : 'text-gray-600 dark:text-gray-400'}`}>{cp.message}</p>
                                        {cp.location && <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{cp.location}</p>}
                                        {cp.time && (
                                          <p className="text-xs text-gray-300 dark:text-gray-600 mt-0.5">
                                            {new Date(cp.time).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' })}, {new Date(cp.time).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                                          </p>
                                        )}
                                      </div>
                                    </div>
