    ...
    <div className="py-8 px-4 lg:px-8 min-h-screen" style={{ background: theme === 'dark' ? '#0f172a' : '#f4f6fb' }}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">Retrouvez tous vos achats par produit ou fournisseur</p>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full pl-12 pr-12 py-3.5 text-sm bg-white dark:bg-gray-850 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm dark:shadow-none focus:outline-none focus:ring-2 focus:ring-blue-400 dark:focus:ring-blue-500"
            autoFocus
          />
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-400 hover:text-gray-600 dark:text-gray-400 dark:text-gray-400">
              <X size={16} />
            </button>
          )}
        </div>
        ...
        {filtered.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            {[
              ...
              const Icon = s.icon;
              return (
                <div key={i} className="bg-white dark:bg-gray-850 rounded-2xl shadow-sm dark:shadow-none p-4 flex items-center gap-3">
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-850 rounded-2xl p-16 text-center shadow-sm dark:shadow-none">
            <p className="text-gray-500 dark:text-gray-400">Aucun résultat pour "<strong>{search}</strong>"</p>
          </div>
        ) : (
          <div className="bg-white dark:bg-gray-850 rounded-2xl shadow-sm dark:shadow-none overflow-hidden">
            {/* Header tableau */}
            <div className="grid grid-cols-12 px-5 py-3 bg-gray-50 dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700 text-xs font-semibold text-gray-400 dark:text-gray-400 uppercase tracking-wide">
                ...
                return (
                  <div key={idx} className="grid grid-cols-12 px-5 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-900 transition-colors items-center">
                    ...
                    <div className="col-span-3 min-w-0">
                      <span className="text-sm text-gray-500 dark:text-gray-400 truncate block">{highlight(line.supplierName)}</span>
                    </div>
                    <div className="col-span-1 text-center">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-700">{line.quantity}</span>
                    </div>
                    <div className="col-span-1 text-right">
                      <span className="text-xs text-gray-400 dark:text-gray-400">{fmtPrice(line.unitPrice)}</span>
                    </div>
                    <div className="col-span-1 text-right">
                      <span className="text-sm font-semibold text-blue-600">{fmtPrice(line.totalPrice)}</span>
                    </div>
                    <div className="col-span-1 text-right">
                      <span className="text-xs text-gray-400 dark:text-gray-400 flex items-center justify-end gap-1">
            ...
            <div className="px-5 py-3 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
              <span className="text-xs text-gray-400 dark:text-gray-400">{filtered.length} résultat{filtered.length > 1 ? 's' : ''}</span>
