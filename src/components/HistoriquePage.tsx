    ...
    <div className="py-8 px-4 lg:px-8 min-h-screen" style={{ background: '#f4f6fb' }}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 dark:text-gray-500 mt-1 text-sm">Retrouvez tous vos achats par produit ou fournisseur</p>
        </div>

        {/* Barre de recherche */}
        <div className="relative mb-6">
          <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-600" />
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Rechercher un produit ou un fournisseur..."
            className="w-full pl-12 pr-12 py-3.5 text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm dark:shadow-none focus:outline-none focus:ring-2 focus:ring-blue-400 dark:focus:ring-blue-500"
            autoFocus
          />
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-600 hover:text-gray-600 dark:text-gray-400 dark:text-gray-600">
              <X size={16} />
            </button>
          )}
        </div>
        ...
        {filtered.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            {[
              ...
              const Icon = s.icon;
              return (
                <div key={i} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm dark:shadow-none p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-bold text-gray-900 dark:text-gray-100">{s.val}</p>
                  </div>
                </div>
              );
            })}
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-16 text-center shadow-sm dark:shadow-none">
            ...
            <p className="text-lg font-semibold text-gray-700 dark:text-gray-300 dark:text-gray-700">Tapez pour rechercher</p>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-16 text-center shadow-sm dark:shadow-none">
            <p className="text-gray-500 dark:text-gray-500">Aucun résultat pour "<strong>{search}</strong>"</p>
          </div>
        ) : (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm dark:shadow-none overflow-hidden">
            {/* Header tableau */}
            <div className="grid grid-cols-12 px-5 py-3 bg-gray-50 dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700 text-xs font-semibold text-gray-400 dark:text-gray-600 uppercase tracking-wide">
            ...
            <div className="divide-y divide-gray-50">
              {filtered.map((line, idx) => {
                ...
                return (
                  <div key={idx} className="grid grid-cols-12 px-5 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-900 transition-colors items-center">
                    <div className="col-span-5 flex items-center gap-2.5 min-w-0">
                      ...
                      <span className="text-sm text-gray-800 dark:text-gray-200 truncate">{highlight(line.productName)}</span>
                    </div>
                    <div className="col-span-3 min-w-0">
                      <span className="text-sm text-gray-500 dark:text-gray-500 truncate block">{highlight(line.supplierName)}</span>
                    </div>
                    <div className="col-span-1 text-center">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-700">{line.quantity}</span>
                    </div>
                    <div className="col-span-1 text-right">
                      <span className="text-xs text-gray-400 dark:text-gray-600">{fmtPrice(line.unitPrice)}</span>
                    </div>
                    <div className="col-span-1 text-right">
                      <span className="text-sm font-semibold text-blue-600">{fmtPrice(line.totalPrice)}</span>
                    </div>
                    <div className="col-span-1 text-right">
                      <span className="text-xs text-gray-400 dark:text-gray-600 flex items-center justify-end gap-1">
            ...
            <div className="px-5 py-3 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
              <span className="text-xs text-gray-400 dark:text-gray-600">{filtered.length} résultat{filtered.length > 1 ? 's' : ''}</span>
